...
import ShareComponets from './Share';
...
const BasicCard = () => {
 ...
 const handleFavorite = (item) => {
  let itemArray;
  const itemExists = favData.some(fav => fav.id === item.id);
  if (!itemExists) {
 ...
 useEffect(() => {
  ...
  const getFavList = localStorage.getItem("favoriteList");
  if (getFavList) {
   setFavData(JSON.parse(getFavList));
  }
 }, []);
      ...
      <Grid key={item?.id} item xs={12} sm={6} md={3} lg={3} xl={2}>
       <Card sx={{ maxWidth: 345 }}>
        ...
        <CardActions disableSpacing>
         ...
         <ShareComponets
          shareUrl={`${String(window.location)}product/${item?.id}`}
          title={`${item?.title}`}
         >
          <IconButton aria-label="share">
           <ShareIcon />
          </IconButton></ShareComponets>
         <ExpandMore
          expand={expanded}
